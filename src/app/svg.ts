...
export interface SvgStyleTransfer {
  key: string;
  attribute: string;
  newValue: string;
}
...
class HorizontalLineTo extends SvgItem {
  ...
  public override rotate(ox: number, oy: number, angle: number, force: boolean = false) {
    if (angle == 180) {
...
class EllipticalArcTo extends SvgItem {
  ...
  public override rotate(ox: number, oy: number, degrees: number, force: boolean = false) {
  ...
  public override asString(decimals: number = 4, minify: boolean = false, trailingItems: SvgItem[] = []): string {
    if (!minify) {
